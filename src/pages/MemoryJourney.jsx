...
/* ═══════════════════════════════════════════════════════════════════════════
   LETTER DATA — replace the body text between [ ] with your personal words
   ═══════════════════════════════════════════════════════════════════════════ */
const LETTERS = [
  {
    id: 'birthday',
...
function Tape({ rotate }) {
  return (
    <div style={{
      ...
      width: '48px',
      height: '14px',
      backgroundColor: 'rgba(217,147,139,0.20)',
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {/* Rule */}
        <div style={{ width: '28px', height: '1px', backgroundColor: 'var(--border-color)', marginTop: '4px' }} />
...
/* ─── Full-screen fixed overlay when a letter is open ───────────────────── */
...
/* ─── Full-screen fixed overlay when a letter is open ───────────────────── */
function LetterOverlay({ letter, onClose }) {
  ...
  return (
    ...
    >
      {/* Paper sheet — stop click propagation so only backdrop tap closes */}
      <motion.div
        key="overlay-paper"
      ...
      >
        ...
        <div style={{
          ...
          boxShadow: '0 12px 40px rgba(138,124,119,0.20), 0 2px 8px rgba(138,124,119,0.10)',
          position: 'relative',
          overflow: 'hidden',
        }}>
          {/* Paper dot texture */}
          ...
          {Array.from({ length: 120 }).map((_, i) => (
            <div key={i} style={{
              ...
              top: `${94 + i * 26}px`, height: '1px',
              backgroundColor: 'rgba(188,170,160,0.10)',
              pointerEvents: 'none',
            }} />
          ))}
          ...
          <div style={{ textAlign: 'center', marginBottom: '22px', position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ width: '36px', height: '1px', backgroundColor: 'var(--border-color)', margin: '10px auto 0' }} />
          </div>
          ...
          <div style={{ textAlign: 'right', position: 'relative', zIndex: 1 }}>
            ...
            <span style={{ fontSize: '1.2rem', color: letter.accent }}>{letter.signature}</span>
          </div>
...
/* ═══════════════════════════════════════════════════════════════════════════
   MAIN PAGE
   ═══════════════════════════════════════════════════════════════════════════ */
export default function MemoryJourney({ onNextPage }) {
  const [collageCompleted, setCollageCompleted]   = useState(false);
  const [showBook, setShowBook]                   = useState(false);
  const [bookFinished, setBookFinished]           = useState(false);
  const [openLetterId, setOpenLetterId]           = useState(null); // null | 'birthday' | 'thankyou'
  ...
  // When book appears, smooth scroll down to it
  const handleTurnPageClick = () => {
    setShowBook(true);
    setTimeout(() => {
      const bookSec = document.getElementById('memory-book-section');
      if (bookSec) bookSec.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 150);
  };

  return (
    <>
      {/* ── OPEN LETTER OVERLAY (fixed, preserves scroll position) ── */}
      <AnimatePresence>
        {openLetter && (
          <LetterOverlay
            key={openLetter.id}
      ...
      >
        {/* ── PART A: FOUR MAIN MEMORIES ── */}
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          {/* Music Card */}
          <MusicCard />

          {/* Memory Intro */}
          <div style={{ textAlign: 'center', margin: '0.75rem 0 2rem 0' }}>
            <h2 style={{
              fontFamily: 'var(--font-serif)',
            ...
            <p style={{
              ...
              "Some memories are worth uncovering."
            </p>
            <p style={{
              fontFamily: 'var(--font-sans)',
              fontSize: '0.65rem',
          ...
          <MemoryCollage onAllRevealed={() => setCollageCompleted(true)} />
        </div>

        {/* ── QUIET TRANSITION TO MEMORY BOOK ── */}
        <AnimatePresence>
          {collageCompleted && (
            <motion.div
        ...
        {/* ── PART B: THE MEMORY BOOK ── */}
        <AnimatePresence>
          {showBook && (
            <motion.div
              id="memory-book-section"
            ...
            >
              ...
              <MemoryBook onFinish={() => setBookFinished(true)} />
            </motion.div>
          )}
        </AnimatePresence>

        {/* ── PART C: BOOK OUTRO + LETTERS (continuous scroll) ── */}
        <AnimatePresence>
          {bookFinished && (
            <motion.div
              id="letters-section"
            ...
            >
              ...
              <div style={{
                ...
                <span style={{ fontSize: '1rem', color: 'var(--accent-pink)', opacity: 0.6, letterSpacing: '6px' }}>🌸 ♡ 🌸</span>
              ...
              {/* ── Two envelope cards side-by-side ── */}
              <div style={{
                display: 'flex',
                flexDirection: 'row',
                justifyContent: 'center',
                ...
                {LETTERS.map(letter => (
                  <EnvelopeCard
                    key={letter.id}
                    letter={letter}
                    onOpen={() => setOpenLetterId(letter.id)}
                  />
                ))}
              </div>
